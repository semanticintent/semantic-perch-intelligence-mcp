...
export type MigrationDirection = 'dev_to_staging' | 'staging_to_production' | 'dev_to_production' | 'reverse' | 'none';

export interface ContextFactors {
  environmentCriticality?: number; // 0-10: How critical is this environment?
  migrationRisk?: number; // 0-10: Risk of schema migration
  dependencyComplexity?: number; // 0-10: How many dependencies are affected?
  operationalImpact?: number; // 0-10: Downtime, rollback difficulty
  changeFrequency?: number; // 0-10: How often does this schema change?
}

export class ContextAnalysis {
  public readonly score: number;
  public readonly environment: Environment | null;
  public readonly factors: ContextFactors;
  public readonly reasoning: string;

  constructor(factors: ContextFactors, reasoning: string, environment: Environment | null = null) {
   ...
   */
  static forEnvironment(environment: Environment, hasDependencies: boolean): ContextAnalysis {
    const envScores = {
    ...
    return new ContextAnalysis(
      ...
      `Optimization in ${environment} environment${hasDependencies ? ' with dependencies' : ''}`,
      environment
    );
  }
  ...
  static forSchemaMigration(
    ...
    return new ContextAnalysis(
      ...
      `Schema migration from ${sourceEnv} to ${targetEnv} with ${differencesCount} difference(s)`,
      targetEnv
    );
  }
   ...
   */
  static forMissingPrimaryKey(environment: Environment, isReferencedByOthers: boolean): ContextAnalysis {
    ...
    return new ContextAnalysis(
      ...
      `Missing primary key in ${environment}${isReferencedByOthers ? ' (table is referenced by others)' : ''}`,
      environment
    );
  }
   ...
   */
  static forMissingForeignKeyIndex(
    environment: Environment,
    isFrequentlyQueried: boolean,
    tableCount: number
  ): ContextAnalysis {
    const envCriticality = this.getEnvironmentCriticality(environment);
    ...
    return new ContextAnalysis(
      ...
      `Missing index in ${environment}${isFrequentlyQueried ? ' (frequently queried)' : ''}`,
      environment
    );
  }
   ...
   */
  static forNullableForeignKey(environment: Environment): ContextAnalysis {
    const envCriticality = this.getEnvironmentCriticality(environment);
    ...
    return new ContextAnalysis(
      ...
      `Nullable foreign key in ${environment} - likely intentional design`,
      environment
    );
  }
   ...
   */
  private static getEnvironmentCriticality(env: Environment): number {
    const scores = {
      production: 10,
      staging: 7,
      development: 4,
   ...
   */
  private static getMigrationDirection(source: Environment, target: Environment): MigrationDirection {
    if (source === 'development' && target === 'staging') return 'dev_to_staging';
    if (source === 'staging' && target === 'production') return 'staging_to_production';
    if (source === 'development' && target === 'production') return 'dev_to_production';
    return 'reverse';
  }
   ...
   */
  private static calculateMigrationRisk(direction: MigrationDirection, differencesCount: number): number {
    const baseRisk = {
      dev_to_staging: 5,
